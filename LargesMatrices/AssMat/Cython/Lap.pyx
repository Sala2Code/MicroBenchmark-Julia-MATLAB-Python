...
cdef void StBuffer(Buffer, i, j, Val): # StackBuffer
    Buffer.ij[i+1, j+1] = 1
    Buffer.Val[i+1, j+1] += Val


@cython.boundscheck(False)
cdef void MyQx(Buffer, Val, I, J):
...
def LapParaSym(Mat11,Mat12,Mat21,Mat22, Dx, Dy, Nxy):
    ...
    # 0.0625 = 1/16
    cdef double SqDx = 0.0625 / (Dx * Dx)
    cdef double SqDy = 0.0625 / (Dy * Dy)
    cdef double DxDy = 0.0625 / (Dy * Dx)

    V=np.zeros((3,(Nx-2)*Ny*9))

    Buffer = ThisBuff()
    Val = np.zeros((2,1)) 
    cdef Py_ssize_t iCpt = 0 
